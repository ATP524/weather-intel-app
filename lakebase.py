...
_SCOPE = os.environ.get("LAKEBASE_SECRET_SCOPE", "database-day2")
...
def get_connection():
    ...
    Sets search_path to 'weather, public' so unqualified table references
    resolve to the `weather` schema first (where all app tables live — see
    sql/01_setup_weather_documents.sql), then fall back to public (where the
    pgvector extension lives) if not found.
    """
    conn = psycopg2.connect(_lakebase_url(), cursor_factory=RealDictCursor)
    try:
        # Resolve unqualified names against the `weather` schema first.
